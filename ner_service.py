...
def is_bad_span(text: str) -> bool:
    ...
    if "@" in t or "http" in t or "www" in t: return True
    if re.search(r"\d", t): return True
    if not re.search(r"[aeiouAEIOU]", t): return True
    if any(word.lower() in TITLE_WORDS for word in t.split()): return True
    if any(word.lower() in ORG_WORDS for word in t.split()): return True
    if any(word.lower() in ADDRESS_WORDS for word in t.split()): return True
    if re.search(r"[,\"'&]", t): return True
    if t.isupper(): return True
    return False
